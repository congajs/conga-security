...
class Firewall {
     ...
     * @returns {String|null}
     */
    getRealmForRequest(request = null, controller = null, action = null) {
    ...
    processRequest(request = null, controller = null, action = null) {
        return new Promise((resolve, reject) => {
            ...
            let fail = reject;
            if (stopwatch) {
                stopwatch = stopwatch.request(request, 'security.firewall');
                fail = err => {
            ...
            if (firewall.anonymous || !firewall.authenticator || !firewall.provider) {
                ...
                if (firewall.anonymous ||
                    validateRoles(firewall.roles || [], (token && token.roles) || [])
                ) {
                    stopwatch && stopwatch.ensureStopped();
                    resolve(token || new PreAuthToken());
                } else {
            ...
            if (security.authenticators && firewall.authenticator in security.authenticators) {
                if (typeof security.authenticators[firewall.authenticator] === 'string') {
                    // service is being referenced by SID string
                    let sid = security.authenticators[firewall.authenticator].replace(/^@/, '');
                    if (container.has(sid)) {
                        authenticator = container.get(sid);
                    }
                } else {
                    // service instance was referenced in DI
                    authenticator = security.authenticators[firewall.authenticator];
                }
            }

            if (!(authenticator instanceof AbstractAuthenticator)) {
                throw new ConfigurationError(
                    'Security firewall authenticator not found or is invalid.');
            }

            // get and validate the provider
            const provider = container.get('security.firewall.provider.factory')
            ...
            const finish = (token) => {
                ...
                if (!token ||
                    !token.authenticated ||
                    !validateRoles(firewall.roles, token.roles)
                ) {
                    fail(new AccessDeniedError());
                    return;
                }
