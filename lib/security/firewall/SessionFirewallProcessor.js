...
class SessionFirewallProcessor extends FirewallProcessor {
    ...
    getPreAuthToken() {
        return new Promise((resolve, reject) => {
            ...
            super.getPreAuthToken().then(token => {
                ...
                this.getSessionAuthToken().catch(err => {
                ...
                this.getSessionAuthToken().then(resolve).catch(() => {
    ...
    getSessionAuthToken(sessions = null) {
        ...
        return this.authenticator.refreshToken(token, provider, this.realm).then(token => {
    ...
    process() {
        return new Promise((resolve, reject) => {
            super.process().then(authToken => {
                ...
                } else if (!(authToken instanceof AnonAuthToken)) {
    ...
    processCatcher(err) {
        ...
        // if it's a redirect or a server error, etc. don't clear the session
        if (err.status) {
