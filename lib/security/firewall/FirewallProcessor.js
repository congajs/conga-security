...
class FirewallProcessor {
     ...
     */
    process() {
        // pull out some members from our instance
        const { firewall, authenticator, realm, routes, request } = this;
        ...
        if (this.isLogoutRoute()) {
            const err = new AccessDeniedError('Unauthorized', 401);
        ...
        // if it's an anonymous route, allow access
        if (this.isRedirectRoute() || (authenticator && authenticator.isAnonymousRoute(request))) {
            return Promise.resolve(new AnonAuthToken(realm));
        }

        // resolve a pre-auth token and then authenticate it
        return this.getPreAuthToken()
            .then(preAuthToken => this.authenticateToken(preAuthToken, realm))
