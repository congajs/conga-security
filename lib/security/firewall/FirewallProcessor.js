...
class FirewallProcessor {
    ...
    constructor(request, firewall) {
        ...
        this.realm = firewall && (firewall.realm || firewall.isolatedRealm || firewall.__key);
    ...
    /**
     * Getter to see if the firewall is simple (doesn't have an authenticator and / or provider)
     * @returns {Boolean}
     */
    get isSimpleFirewall() {
        return !this.authenticator || !this.provider;
    }
    ...
    getPreAuthToken() {
        const stopwatch = this.stopwatch &&
            this.stopwatch.start('firewall.pre_authenticate', 'security');
        ...
        return this.authenticator.createToken(this.request, this.realm).then(preAuthToken => {
            stopwatch && stopwatch.stop();
            return preAuthToken;
        }).catch(err => {
            stopwatch && stopwatch.stop();
    ...
    authenticateToken(token, realm) {
        const stopwatch = this.stopwatch &&
            this.stopwatch.start('firewall.authenticate', 'security');
        ...
        return this.authenticator.authenticateToken(token, this.provider, realm).then(token => {
            stopwatch && stopwatch.stop();
            return token;
        }).catch(err => {
            stopwatch && stopwatch.stop();
    ...
    validateToken(token) {
        ...
        const stopwatch = this.stopwatch &&
            this.stopwatch.start('firewall.validate', 'security');
    ...
    process() {
        const stopwatch = this.stopwatch &&
            this.stopwatch.start('firewall.processor', 'security');
        ...
        if (this.isLogoutRoute()) {
            ...
            let promise = this.processCatcher(err);
            stopwatch && stopwatch.stop();
            return promise;
        }
    ...
    /**
     * Catch errors from processing a request (normal errors and 401, 403, 302, 500 http status codes)
    ...
    processCatcher(err) {
        ...
        if (!err.headers || !('location' in err.headers)) {
            if (!this.isRedirectRoute() && this.routes.redirect) {
                const headers = err.getResponseHeaders && err.getResponseHeaders();
                err = new HttpError(err.message, 302, err);
                if (headers) {
                    for (const header in headers) {
                        // NOTE: we don't include www-authenticate headers because the URL changed
                        if (header !== 'www-authenticate') {
                            err.addResponseHeader(header, headers[header]);
                        }
                    }
                }
