...
class BassProvider extends AbstractProvider {
    ...
    constructor(container, config) {
        ...
        if (!this.manager) {
            throw new ConfigurationError(
                'No manager was found for document: ' + config.bass.document);
        }
        ...
        if (!metadata) {
            throw new ConfigurationError(
                'Unable to find mapped metadata for document: ' + config.bass.document);
        }
    ...
    /**
     * {@inheritDoc}
     * @see AbstractProvider.refreshResource
     */
    refreshResource(resource) {
        ...
        return this.manager.find(this.documentName, resource[this.documentIdField]).then(refreshed => {
            ...
            if (changePasswordLogout === undefined || changePasswordLogout) {
                ...
                const alen = resource[this.config.bass.secret].length;
                const blen = refreshed[this.config.bass.secret].length;
                const a = Buffer.from(resource[this.config.bass.secret]);
                const b = Buffer.from(refreshed[this.config.bass.secret]);
                if (alen !== blen || !crypto.timingSafeEqual(a, b)) {
                    return Promise.resolve(null);
                }
            }

            // resolve with the new refreshed resource
            return Promise.resolve(new AuthResourceProxy(refreshed));
        });
    }
}
