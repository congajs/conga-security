...
class EncryptionFactory {
    ...
    registerAlgorithm(algo, encryptor) {
        ...
        if (!(encryptor instanceof AbstractEncryptor) &&
            (!(encryptor.encrypt instanceof Function) || !(encryptor.compare instanceof Function))
        ) {
    ...
    getEncryptorForConfig(config) {
        ...
        if (!encryptor && config.id) {
            const sid = config.id.replace(/^@/, '');
            if (this.container.has(sid)) {
                encryptor = this.container.get(sid);
            }
        }
