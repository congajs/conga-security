        ...
        for (let annotation of reader.methodAnnotations) {
            if (annotation instanceof FirewallAnnotation) {
                if (!annotation.provider || !annotation.authenticator) {
                    throw new ConfigurationError('Your @Firewall annotation is not configured correctly.');
                }
                ...
                for (let i in routes) {
                    if (routes[i].action === annotation.target) {
                        ...
                        if (!(realm in security.firewall)) {
                            if (!security.authenticators[annotation.authenticator]) {
                                if (annotation.authenticator[0] !== '@') {
                                    throw new ConfigurationError('Your @Firewall annotation has an invalid authenticator.');
                                }
                            ...
                            if (!security.providers[annotation.provider]) {
                                if (annotation.provider[0] !== '@') {
                                    throw new ConfigurationError('Your @Firewall annotation has an invalid provider.');
                                }
                                security.providers[annotation.provider] = annotation.provider;
                            }
                            security.firewall[realm] = Object.assign({}, annotation, {
                                realm,
                                route: annotation.route || routes[i].path.replace(/:[^\/]+/g, '[^/]+'),
                                controller: routes[i].controller,
                                action: routes[i].action
                            });
                            methodRoutes[i] = security.firewall[realm];
                        }
                        break;
                    }
                }
            ...
            if (annotation instanceof FirewallAnnotation) {
                ...
                if (annotation.route) {
                    ...
                    if (!(realm in security.firewall)) {
                        ...
                        security.firewall[ realm ] = Object.assign({}, annotation, { realm });
                    }
                }
                ...
                for (let i in routes) {
                    ...
                    if (!(i in methodRoutes)) {
                        ...
                        if (!(useRealm in security.firewall)) {
                            if (!security.authenticators[annotation.authenticator]) {
                                if (annotation.authenticator[0] !== '@') {
                                    throw new ConfigurationError('Your @Firewall annotation has an invalid authenticator.');
                                }
                            ...
                            security.firewall[useRealm] = Object.assign({}, annotation, {
                                realm: useRealm,
                                route: annotation.route || routes[i].path.replace(/:[^\/]+/g, '[^/]+'),
                                controller: routes[i].controller,
                                action: routes[i].action
